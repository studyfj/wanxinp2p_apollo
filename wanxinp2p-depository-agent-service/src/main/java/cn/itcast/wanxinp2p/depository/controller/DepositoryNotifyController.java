 ...
 * @date 2022/7/12 20:12
 * @Description 接受银行存管的最大努力通知
 */
...
public class DepositoryNotifyController {
    ...
    public String receiveDepositoryCreateConsumerResult(@RequestParam("serviceName") String serviceName, @RequestParam("platformNo") String platformNo, @RequestParam("signature") String signature, @RequestParam("reqData") String reqData) {
        //1.更新数据
        // 银行返回是base64编码,先解码在转换java对象
        String str = EncryptUtil.decodeUTF8StringBase64(reqData);
        DepositoryConsumerResponse depositoryConsumerResponse = JSON.parseObject(str, DepositoryConsumerResponse.class);
        depositoryRecordService.modifyRequestStatus(depositoryConsumerResponse.getRequestNo(), depositoryConsumerResponse.getStatus());
        //2.给用户中心发送消息
        gatewayMessageProducer.personRegister(depositoryConsumerResponse);
        //3.给银行存管系统返回结果
        return "OK";
    }
}
